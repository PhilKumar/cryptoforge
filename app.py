    ...
    if key == "COINDCX_BASE_URL":
        if raw:
            ...
            if parsed.scheme not in {"http", "https"} or not parsed.netloc:
                raise HTTPException(
                    status_code=400, detail="COINDCX_BASE_URL must start with https:// and include a host"
                )
        return raw or "https://api.coindcx.com"
    if key == "COINDCX_PUBLIC_URL":
        if raw:
            parsed = urlparse(raw)
            if parsed.scheme not in {"http", "https"} or not parsed.netloc:
                raise HTTPException(
...
async def get_portfolio_summary():
    ...
    try:
        ...
        return {
            ...
            "balance": _safe_float(accounting.get("wallet_equity", {}).get("usd"), usdt_balance),
            "wallet_balance": _safe_float(accounting.get("wallet_balance", {}).get("usd"), usdt_balance),
            "wallet_equity": _safe_float(accounting.get("wallet_equity", {}).get("usd"), usdt_balance),
